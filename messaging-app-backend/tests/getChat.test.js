...
test("Retrieve chat with correct info", async () => {
  const authorization = {
    Authorization: `Bearer ${token}`,
  };

  const res = await request(app)
    .get("/chat/621ff30d2a3e781873fcb669")
    .set("Content-Type", "application/json")
    .set(authorization)
    .then((res) => {
      expect(res.body.otherUser._id).toEqual("621ff30d2a3e781873fcb664");
      expect(res.body.chat._id).toEqual("621ff30d2a3e781873fcb669");
    });
});

test("Returns error if chat not found", async () => {
  const authorization = {
    Authorization: `Bearer ${token}`,
  };

  const res = await request(app)
    .get("/chat/621ff30d2a3e781873fcb679")
    .set("Content-Type", "application/json")
    .set(authorization)
    .then((res) => {
